...
time_interval = """(
SELECT MIN(start_time), MAX(start_time) FROM time
)"""

most_repeated_locations = """(
SELECT location, COUNT(location) AS most_repeated_location 
FROM artists
GROUP BY location
ORDER BY most_repeated_location DESC
LIMIT    5
)"""

years_with_most_productivity = """(
SELECT year, COUNT(year) AS occurences_in_year
FROM songs
GROUP BY year
ORDER BY occurences_in_year DESC
LIMIT    5
)"""

payment_analysis = """(
SELECT level, COUNT(level) AS counter
FROM users
GROUP BY level
...
analytics_queries = [count_songplays, time_interval, most_repeated_locations, years_with_most_productivity, payment_analysis]